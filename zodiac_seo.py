...
def target_iso() -> str:
    ...
    ZODIAC_TARGET_DATE=YYYY-MM-DD가 있으면 스케줄러가 고정한 날짜를 최우선으로 쓴다.
    이는 GitHub Actions가 자정을 넘겨 지연 실행될 때 preflight와 실제 발행 날짜가
    하루씩 어긋나는 것을 막는다.
    ...
    """
    explicit = os.environ.get("ZODIAC_TARGET_DATE", "").strip()
    if explicit:
        # Validate instead of silently accepting malformed scheduler input.
