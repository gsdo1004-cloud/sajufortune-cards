...
class ConsumerQueueTests(unittest.TestCase):
 ...
 def test_consumer_verified_completes(self):
  ...
  oi,os,or_=c.input_exact,c.submit_once,c.restore_keyboard
  try:
   c.input_exact=lambda *a:c.Result('COMPOSED','exact'); c.submit_once=lambda *a:c.Result('VERIFIED','receipt'); c.restore_keyboard=lambda *a:None
   r=c.process_queue_once('SER','acct','2026-01-01T00:00:01+00:00'); self.assertEqual(r.status,'VERIFIED'); self.assertEqual(q.load()[0]['status'],'PUBLISHED')
  finally: c.input_exact,c.submit_once,c.restore_keyboard=oi,os,or_
 def test_consumer_unverified_is_terminal(self):
  ...
  oi,os,or_=c.input_exact,c.submit_once,c.restore_keyboard
  try:
   c.input_exact=lambda *a:c.Result('COMPOSED','exact'); c.submit_once=lambda *a:c.Result('FAILED_UNVERIFIED','no_retry'); c.restore_keyboard=lambda *a:None
   r=c.process_queue_once('SER','acct','2026-01-01T00:00:01+00:00'); self.assertEqual(r.status,'FAILED_UNVERIFIED'); self.assertEqual(q.load()[0]['status'],'FAILED_UNVERIFIED')
  finally: c.input_exact,c.submit_once,c.restore_keyboard=oi,os,or_
